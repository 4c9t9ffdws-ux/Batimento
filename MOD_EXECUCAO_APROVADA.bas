...
Public Sub ExecutarBatimentoInventarioAprovado()
    If Not ConfirmarExecucaoDaPrevia("Batimento de Inventario", "INVENTARIO") Then Exit Sub
...
Public Sub ExecutarBatimentoDiarioAprovado()
    If Not ConfirmarExecucaoDaPrevia("Batimento Diario", "DIARIO") Then Exit Sub
    batimento_lote_nivel_diario
End Sub

Public Sub ExecutarBatimentoBloqueioAprovado()
    If Not ConfirmarExecucaoDaPrevia("Batimento de Bloqueio", "BLOQUEIO") Then Exit Sub
    sb_Batimento_Status_BLOQUEIO
End Sub

Public Sub ExecutarBatimentoDesbloqueioAprovado()
    If Not ConfirmarExecucaoDaPrevia("Batimento de Desbloqueio", "DESBLOQUEIO") Then Exit Sub
    sb_Batimento_Status_desbloqueio
End Sub

Private Function ConfirmarExecucaoDaPrevia(ByVal titulo As String, ByVal tipoEsperado As String) As Boolean
    ...
    If ws_base.Range("AI4").Value <> tipoEsperado Then
        MsgBox "A previa atual nao pertence a este fluxo. Gere a previa de " & titulo & ".", vbExclamation, titulo
        Exit Function
    End If
